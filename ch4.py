...
#        self.wave_offset = wave_offset
    ...
    def methane_only_spectrum(self) : 
        ch4 = self.ch4_fun[self.whw] * self.fun_scaling + self.ch4_hot[self.whw] * self.hot_scaling
        return ch4
...
#        self.whw = np.argwhere((self.wave >= np.min(wave)) & (self.wave <= np.max(wave)))
    ...
    def fit(self, wave, spec, width = 0.06, start = 3.36, end = 3.56) : 
        '''
        ...
        self.methane_only = spec.copy()
        self.methane_only[:] = np.nan
...
#            whw = np.argwhere((wave > r) & (wave < r + width)).flatten()            
            subwave = wave[whl]
            subspec = spec[whl]
            fit = self.fit_subregion(subwave, subspec)
            ret[whl] = fit
            self.methane_only[whl] = self.methane_only_spectrum()
